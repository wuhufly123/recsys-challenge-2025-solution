LOG_NAME="train_features_epoch3_churn"
...
python -m ubt_solution.create_embeddings \
    ...
    --embeddings-dir /data_sdc/mhwang/recsys_code/submit_file/train_features_epoch3_churn \
    --accelerator cuda \
    --devices 0 \
    --num-workers 4 \
    --batch-size 128 \
    --num-epochs 3 \
    --learning-rate 1e-4 \
    --task-weights "churn:1.0,category_propensity:0.0,product_propensity:0.0,price:0.0" \
    --seed "$SEED" \
    > >(tee -a "$LOG_FILE") 2>&1
# 检查命令执行是否成功
if [ $? -eq 0 ]; then
